                ...
                <div class="config_step" data-step="{$step->id}" {if $k>0}style="display:none" {/if}>
                    ...
                    {foreach $step->options as $opt}
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="option[{$step->id}]" value="{$opt->id}"
                                data-price="{$opt->price}" id="opt{$opt->id}" />
                            <label class="form-check-label" for="opt{$opt->id}">{$opt->name} {if $opt->price>0}<span
                                    class="text-muted">+{$opt->price}</span>{/if}</label>
                        </div>
                    {/foreach}
