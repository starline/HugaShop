...
<div class="mt-4">
    ...
    <div class="btn_row left">
        <a class="btn btn-light"
            href="{'PaymentExchange'|urll:[id => $payment_method->id]}?order_url={$order->url}&form_type=invoice"
            target="_blank" data-bs-toggle="tooltip" title="Сформировать Счет">Счет</a>
        <a class="btn btn-light"
            href="{'PaymentExchange'|urll:[id => $payment_method->id]}?order_url={$order->url}&form_type=packing_list"
            data-bs-toggle="tooltip" target="_blank" title="Сформировать Расходную накладную">Накладная</a>
        <a class="btn btn-light"
            href="{'PaymentExchange'|urll:[id => $payment_method->id]}?order_url={$order->url}&form_type=commercial_offer"
