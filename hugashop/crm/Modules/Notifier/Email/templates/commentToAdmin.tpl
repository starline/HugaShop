...
{if $comment->approved}
  <h1 style="font-weight:normal;"><a href="{$config->root_url}/admin/comment/{$comment->id}">Новый
      комментарий</a> от {$comment->name}
  </h1>
{else}
  <h1 style="font-weight:normal;"><a href="{$config->root_url}/admin/comment/{$comment->id}">Комментарий</a>
    от {$comment->name} ожидает одобрения</h1>
{/if}
