Заказ <b><a href="{$config->root_url}{'OrderAdmin'|urll:[id => $order->id]}">№{$order->id}</a></b> - {$order->date|date} {$order->date|time}{PHP_EOL}
<b>Заказчик:</b>
...
{$url = "{$config->root_url}{'OrderAdmin'|urll:[id => $order->id]}"}{$url_text = "Открыть заказ на сайте"}