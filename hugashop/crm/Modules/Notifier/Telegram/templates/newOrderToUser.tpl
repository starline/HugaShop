Ваш заказ <b><a href="{$config->root_url}{'Order'|urll:[id => $order->id, order_url => $order->url]}">№{$order->id}</a></b>{PHP_EOL}
<b>Получатель:</b>
...
{$url = "{$config->root_url}{'Order'|urll:[id => $order->id, order_url => $order->url]}"}{$url_text = "Открыть заказ на сайте"}