            ...
            <ul>
                ...
                {foreach item=c from=$categories}
                    <li data-category="{$c->id}" class="
                        ...
                        <div category_id="{$c->id}" class="link category">
                            <a href="{url keyword=null category_id=$c->id clear=true}" data-bs-toggle="tooltip"
                                {if $c->name|count_characters:true > 25}title="{$c->name}"
                                {/if}>{$c->name|truncate:25:' …':false:false}</a>
                        </div>
