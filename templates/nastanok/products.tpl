		...
		<ul itemscope itemtype="https://schema.org/BreadcrumbList">
			...
			{if $category}
				{foreach $category->path as $cat}
					<li class='arrow'>/</li>
					<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
						<a href="/{$cat->url}" itemprop="item"><span itemprop="name">{$cat->name}</span>
					...
					{foreach $category->path[0]->subcategories as $c}
						{if $c->visible}
							<li class="category_main {if $category->id == $c->id}selected{/if}">
								<a href="{$c->url}" data-category="{$c->id}">{$c->name}</a>
								{if $c->subcategories}
									<ul>
										{foreach $c->subcategories as $sc}
											<li {if $category->id == $sc->id}class="selected" {/if}>
												<a href="{$sc->url}" data-category="{$sc->id}">{$sc->name}</a>
											</li>
										{/foreach}
									</ul>
								{/if}
		...
		<div class="col-lg-9 wrap_products {if !$category}wide{/if}">
			...
			{if 'product_category'|user_access AND $category->id}
				<div class="admin_edit">
					<a href="/admin/product/category/{$category->id}" data-bs-toggle="tooltip"
