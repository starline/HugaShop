									...
									{if $c->subcategories}
										<ul>
											{foreach $c->subcategories as $sc}
												<li>
													<a {if $category->id == $sc->id}class="selected" {/if}
														href="{'Products'|linkLang:[url => $sc->url]}"
		...
		<div class="col-lg-9 wrap_products position-relative">
			...
			<h1>{$h1}
				{if 'product_category'|user_access AND $category->id}
					{include file='parts/btn_edit.tpl' btn_edit_link={'CategoryAdmin'|link:[id => $category->id]}}
				{/if}
			</h1>
						...
						{foreach $features as $f}
							<tr>
								...
								<td class="feature_values">
									<span class="option">
										<a href="{filter_url feature_id=$f->id}" {if !$selected_features[$f->id]}class="selected" {/if}
											rel="nofollow">Все</a>
									</span>
									{foreach $f->options as $o}
										<span class="option">
											<a href="{filter_url feature_id=$f->id option_url=$o->url}"
												class="{if $selected_features[$f->id]==$o->id}selected{/if} {if $o->disabled}disabled{/if}"
