...
class CheckoutController extends BaseFrontController
{
    ...
    public function checkout(): Response
    {
        ...
        if (!empty($pre_order = Secure::getInputAcces($this->fields))) {
            ...
            if (!empty(Request::post('checkout'))) {
                ...
                if (!empty($auth_user->id)) {
                    $pre_order->user_id = $auth_user->id;
                }


                // Clean Cart (session). Prevent create duplicate orders
                Cart::cleanCart();
                ...
                if (!empty($pre_order->phone) and empty($auth_user->id)) {
        ...
        // Данные пользователя
        if (!empty($auth_user)) {
            $last_order = Order::getOrders(['user_id' => $auth_user->id, 'limit' => 1])->first();

            $pre_order = $pre_order ?: new \stdClass();
            ...
            if ($last_order) {
                ...
                $pre_order->delivery_id         = $last_order->delivery_id;
                $pre_order->payment_method_id   = $last_order->payment_method_id;
            } else {
                $pre_order->name     = $auth_user->name;
                $pre_order->email    = $auth_user->email;
                $pre_order->phone    = $auth_user->phone;
            }
        }
